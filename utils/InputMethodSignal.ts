...
function updateInputMethod(
	method: "guess" | "touch-event" | "mouse-event" | "media-query",
	...
	if (
		!forceUpdate && timeSinceLastSwitch < DEBOUNCE_TIME &&
		oldValue !== undefined
	) {
		console.log(
			`Debouncing: Ignoring ${method} switch (${timeSinceLastSwitch}ms since last switch)`,
		);
		return;
	}
	...
	if (forceUpdate || oldValue !== isTouchDevice) {
		...
		console.log(
			`🔄 Input method switched via ${method}: ${
				isTouchDevice ? "Touch" : "Mouse"
			} (was: ${oldValue ? "Touch" : "Mouse"})`,
		);
...
function updateLabels() {
	...
	if (debugLabel) {
		...
		debugLabel.innerHTML = `
			...
			<div><strong>Initial Guess:</strong> ${
			debugInfo.value.initialGuess ? "Touch" : "Mouse"
		}</div>
			<div><strong>Touch Events:</strong> ${debugInfo.value.events.totalTouchEvents} (last: ${
			lastTouchTime ? `${Math.round(timeSinceTouch / 1000)}s ago` : "never"
		})</div>
			<div><strong>Mouse Events:</strong> ${debugInfo.value.events.totalMouseEvents} (last: ${
			lastMouseTime ? `${Math.round(timeSinceMouse / 1000)}s ago` : "never"
		})</div>
			<div><strong>Media Query - Hover:</strong> ${
			debugInfo.value.mediaQueries.hover ? "Yes" : "No"
		}</div>
			<div><strong>Media Query - Coarse Pointer:</strong> ${
			debugInfo.value.mediaQueries.coarsePointer ? "Yes" : "No"
		}</div>
			<div><strong>Has Touch Events:</strong> ${
			"ontouchstart" in globalThis ? "Yes" : "No"
		}</div>
...
export function initInputDetector() {
	...
	touchEvents.forEach((eventType) => {
		globalThis.addEventListener(eventType, () => {
			...
			console.log(
				`👆 Touch event: ${eventType} (#${debugInfo.value.events.totalTouchEvents})`,
			);
	...
	globalThis.addEventListener("mousemove", (e) => {
		...
		if (e.movementX !== 0 || e.movementY !== 0) {
			...
			console.log(
				`🖱️ Mouse movement detected (#${debugInfo.value.events.totalMouseEvents})`,
			);
	...
	globalThis.addEventListener("mousedown", () => {
		...
		console.log(
			`🖱️ Mouse click detected (#${debugInfo.value.events.totalMouseEvents})`,
		);
		updateInputMethod("mouse-event", false);
	}, { passive: true });
